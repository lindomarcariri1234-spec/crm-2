...
export async function expireClientBenefits(
  ...
  const expiredCredits = await tx
    .select()
    .from(financialLedgerEntriesTable)
    .where(and(
      eq(financialLedgerEntriesTable.tenantId, args.tenantId),
      eq(financialLedgerEntriesTable.clientId, args.clientId),
      eq(financialLedgerEntriesTable.direction, "credit"),
      lt(financialLedgerEntriesTable.expiresAt, occurredAt),
    ));

  for (const entry of expiredCredits) {
    if (entry.category !== "wallet" && entry.category !== "cashback") continue;
    await tx.insert(financialLedgerEntriesTable).values({
      id: generateId(),
      tenantId: args.tenantId,
      ...
      idempotencyKey: `benefit-expiry:${entry.id}`,
      reversalOfEntryId: entry.id,
      metadata: { expiredEntryId: entry.id },
      occurredAt,
    }).onConflictDoNothing();
  }
}
