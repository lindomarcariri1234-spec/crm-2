...
for (const file of migrationFiles) {
  applyIncrementalMigrationColumns(
    expected,
    readFileSync(join(__dirname, "../drizzle", file), "utf8"),
  );
}
...
Snapshot/migrations used: ${latestSnapFile} (${expected.size} tables, ${[...expected.values()].reduce((n, s) => n + s.size, 0)} columns expected)
`);
