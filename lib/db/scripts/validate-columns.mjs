...
const alterTableRe =
  /alter table\s+(?:if exists\s+)?"?([a-z][a-z0-9_]*)"?([\s\S]*?);/gi;
const addColRe =
...
for (const file of migFiles) {
  ...
  // Collect every ADD COLUMN in each ALTER TABLE statement, including
  // comma-separated definitions from a single statement.
  let tm;
  while ((tm = alterTableRe.exec(sql)) !== null) {
    const table = tm[1];
    let am;
    while ((am = addColRe.exec(tm[2])) !== null) {
      if (!addedColsByMig.has(table)) addedColsByMig.set(table, new Set());
      addedColsByMig.get(table).add(am[1]);
    }
  }
}
