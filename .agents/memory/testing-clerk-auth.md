...
When using the `testing` skill's `runTest` on VisiteCRM (Clerk auth), pass `testClerkAuth: true`. The `[Clerk Auth] Sign in as {...}` step only works programmatically when this flag is set. If test-session injection returns 422 `form_param_value_invalid` for `redirect_url`, the development URL is not allowed in Clerk; authenticated browser checks remain blocked until that allowlist is corrected.

**Why:** Without the flag, the testing agent tries the real Clerk sign-in/sign-up UI, which is gated by a Cloudflare "Verify you are human" challenge that Playwright can't pass (checkbox not exposed) → test returns `unable`. A correctly flagged test can still fail when Clerk rejects the preview/development redirect URL before establishing a session. There was also a transient `failed_to_load_clerk_js` (proxied `/api/__clerk`) right after an api-server restart — retry/reload handles that.
