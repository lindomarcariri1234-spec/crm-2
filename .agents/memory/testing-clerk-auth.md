...
**How to apply:** `runTest({ testClerkAuth: true, testPlan, relevantTechnicalDocumentation })`. If test-session injection returns 422 for `redirect_url`, first confirm the Vite build receives `CLERK_PUBLISHABLE_KEY`; do not add a temporary preview URL in Clerk. If the running client module contains the injected key and the 422 remains, treat it as a managed test-session/platform blocker and record it as such rather than changing application authentication. To reach a role-gated agency page (e.g. `/insights`, RoleGate AGENCY_ROLES), after sign-in add a `[DB]` step: `UPDATE users SET tenant_id=(SELECT id FROM tenants WHERE slug='demo-agencia'), role='agencia' WHERE email='<login_email>'`, then reload the page so `useGetMe` refetches the role/tenant. `demo-agencia` was a previously-seeded tenant with trips/reservations/clients — check it still exists (`SELECT * FROM tenants WHERE slug='demo-agencia'`) before relying on it, since a dev DB reset can wipe it (seen 2026-08-27, tenants/trips tables were both empty).

**Known flaky failure modes (2026-08-27):** even with `testClerkAuth: true` and an explicit `[Clerk Auth] Sign in as ...` step, the tester has hit a Cloudflare Turnstile block on sign-up and a Clerk "new device" email-verification-code prompt on sign-in that it cannot complete (no mailbox access) — both resolve as `unable`, not an app bug. If this repeats, don't keep retrying the same plan; fall back to non-UI verification (direct `@workspace/db`/`tsx` scripts replicating the query/service logic, or backend-only tests) and note the e2e gap explicitly rather than blocking task completion on it.
