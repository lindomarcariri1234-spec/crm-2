...
## Fix
Strip `sslmode` from the connection string in production and supply explicit SSL options only when TLS is requested. Honor `sslmode=disable`, and when no sslmode is present let node-postgres use its default; this is required for internal PostgreSQL services that do not expose TLS. For exact official Supabase pooler hosts (`pooler.supabase.com` or a subdomain ending in `.pooler.supabase.com`), keep TLS enabled but use `rejectUnauthorized: false`; Vercel's Node runtime otherwise rejects the pooler's managed chain with `SELF_SIGNED_CERT_IN_CHAIN`.
...
**How to apply:** Use the shared database connection configuration for every production PostgreSQL client, including StripeSync. Never relax verification for generic hosts or lookalike suffixes such as `pooler.supabase.com.evil.example`; only an explicit TLS mode or the exact Supabase pooler host should add SSL options.
